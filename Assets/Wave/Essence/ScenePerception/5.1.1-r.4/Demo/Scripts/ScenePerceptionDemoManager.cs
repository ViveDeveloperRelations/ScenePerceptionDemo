...
namespace Wave.Essence.ScenePerception.Sample
{
	public class ScenePerceptionDemoManager : MonoBehaviour
	{
		...
		public Material DemoRoomMaterialOpaque = null, DemoRoomMaterialTransparent = null, GeneratedMeshMaterialTranslucent = null, GeneratedSceneMeshMaterialTranslucent = null;
		public Camera hmd = null;

		public GameObject leftController = null, rightController = null;

		private bool isSceneCMPStarted = false, hideMeshAndAnchors = false;

		public WVR_ScenePerceptionTarget currentPerceptionTarget = WVR_ScenePerceptionTarget.WVR_ScenePerceptionTarget_Max;
		...
		private WVR_SceneMeshType currentSceneMeshType = WVR_SceneMeshType.WVR_SceneMeshType_VisualMesh;
		private WVR_SceneMesh[] currentSceneMeshes = new WVR_SceneMesh[0], prevSceneMeshes = new WVR_SceneMesh[0];
		...
		private void OnEnable()
		{
			...
			if ((Interop.WVR_GetSupportedFeatures() & (ulong)WVR_SupportedFeature.WVR_SupportedFeature_ScenePerception) != 0)
			{
				...
				if (result == WVR_Result.WVR_Success)
				{
					...
					result = scenePerceptionManager.StartScenePerception(currentPerceptionTarget);
		...
		private void Update()
		{ 
			if (isSceneCMPStarted && !hideMeshAndAnchors)
			{
			...
			if (WXRDevice.ButtonPress(WVR_DeviceType.WVR_DeviceType_Controller_Right, WVR_InputId.WVR_InputId_Alias1_B)) //Toggle mesh and anchors
			{
				hideMeshAndAnchors = !hideMeshAndAnchors;
				Log.d(LOG_TAG, "hidePlanesAndAnchors: " + hideMeshAndAnchors);
				if (hideMeshAndAnchors)
				{
					DestroyGeneratedMeshes();
					ClearAnchors();

					currentSceneMeshes = prevSceneMeshes = null;
				}
			}
		}
			...
			if (perceptionStateDictionary[currentPerceptionTarget] == WVR_ScenePerceptionState.WVR_ScenePerceptionState_Completed)
			{
				switch(currentPerceptionTarget)
				{
					...
					case WVR_ScenePerceptionTarget.WVR_ScenePerceptionTarget_SceneMesh:
						{
							result = scenePerceptionManager.GetSceneMeshes(currentSceneMeshType, out currentSceneMeshes);

							if (result == WVR_Result.WVR_Success)
							{
								bool needToUpdateSceneMesh = false;
								if (prevSceneMeshes != null && currentSceneMeshes.Length == prevSceneMeshes.Length) //Update scene mesh
								{
									for (int i=0; i<currentSceneMeshes.Length; i++)
									{
										if (currentSceneMeshes[i].meshBufferId != prevSceneMeshes[i].meshBufferId)
										{
											needToUpdateSceneMesh = true;
											break;
										}
									}
								}
								...
								if (needToUpdateSceneMesh)
								{
									DestroyGeneratedMeshes();

									foreach (WVR_SceneMesh sceneMesh in currentSceneMeshes)
									{
										GameObject generatedSceneMeshGO = scenePerceptionManager.GenerateSceneMesh(sceneMesh, GeneratedSceneMeshMaterialTranslucent, true);
										if (generatedSceneMeshGO)
										{
											generatedSceneMeshGOList.Add(generatedSceneMeshGO);
										}
									}

									prevSceneMeshes = currentSceneMeshes;
								}
							}
							break;
						}
		...
		public void SetSceneMeshToVisual()
		{
			currentSceneMeshType = WVR_SceneMeshType.WVR_SceneMeshType_VisualMesh;
		}

		public void SetSceneMeshToCollider()
		{
			currentSceneMeshType = WVR_SceneMeshType.WVR_SceneMeshType_ColliderMesh;
		}
