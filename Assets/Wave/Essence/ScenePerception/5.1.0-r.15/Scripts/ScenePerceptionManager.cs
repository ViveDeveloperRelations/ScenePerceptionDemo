...
namespace Wave.Essence.ScenePerception
{
	public class ScenePerceptionManager : MonoBehaviour
	{
		...
		/// <summary>
		/// This API is a placeholder and is not ready for use.
		/// The definition of this API may be changed in the future.
		/// </summary>
		public WVR_Result GetSceneMeshes(WVR_SceneMeshType meshType, out WVR_SceneMesh[] meshes)
		{
			...
			{
				//Log.d(LOG_TAG, "WVR_GetSceneMeshes 1 Plane Count Output: " + meshCount);
			}
			...
			for (int i = 0; i < outMeshes.Length; i++)
			{
				...
				offset += Marshal.SizeOf(outMeshes);
			}
		...
		public WVR_Result GetSceneMeshBuffer(UInt64 meshBufferId, out WVR_Vector3f_t[] vertexBuffer, out UInt32[] indexBuffer)
		{
			...
			WVR_Result result = Interop.WVR_GetSceneMeshBuffer(meshBufferId, ref currentBuffer); //Get vertex and index count
			if (result != WVR_Result.WVR_Success)
			{
			...
			currentBuffer.indexBuffer = Marshal.AllocHGlobal(sizeof(UInt32) * (int)currentBuffer.vertexCapacityInput);

			result = Interop.WVR_GetSceneMeshBuffer(meshBufferId, ref currentBuffer); //Get buffers
			if (result != WVR_Result.WVR_Success)
			{
		...
		/// <summary>
		/// A helper function to help create a GeamObject with Mesh related components for showing scene planes in the scene.
		/// </summary>
		...
		/// </param>
		/// <returns>
		/// A GameObject with all the necessary Mesh related components for rendering scene planes in the scene.
		/// </returns>
